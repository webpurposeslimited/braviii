  ...
  return (
    ...
    >
      <div className="flex items-center h-16 px-4 border-b border-slate-800">
        <Link href="/admin" className="flex items-center gap-2">
          ...
          {!collapsed && (
            ...
            >
              <span className="text-lg font-bold text-white">Bravilio</span>
              <span className="text-[10px] font-medium text-blue-400 -mt-1 uppercase tracking-wider">Admin Panel</span>
            </motion.div>
          )}
        </Link>
      </div>
      ...
      <nav className="flex-1 py-4 px-3 space-y-0.5 overflow-y-auto">
        {adminNavItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={cn('h-5 w-5 flex-shrink-0', isActive ? 'text-blue-400' : 'text-slate-500')} />
      ...
      <div className="border-t border-slate-800 py-4 px-3">
        ...
        >
          <ArrowLeft className="h-5 w-5 flex-shrink-0 text-slate-500" />
      ...
      <Button
        variant="ghost"
        size="icon"
        className="absolute -right-3 top-20 h-6 w-6 rounded-full border border-slate-700 bg-slate-900 text-slate-400 hover:text-white"
